...
选项:
  ...
  -q, --quality=NUMBER      设置图像质量，NUMBER 取值范围为 [0.0, 1.0]
  -c, --cache               启用硬盘缓存全部幻灯片模式
\stoptyping

例如启用硬盘缓存、设置图像质量为 0.8，以全屏模式放映文件名为 foo.pdf 的演示文档：

\starttyping
$ cikada -c -q 0.8 -f foo.pdf
\stoptyping

如果未设定图像质量，那么图像质量默认值是 0.5，它表示将原始 PDF 页面转换而成的图像放大二倍。如果图像质量设为 1.0，则表示图像放大了 4 倍。
